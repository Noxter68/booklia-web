...
function BookingCard({
  ...
  const statusConfig: Record<BookingStatus, { label: string; color: string }> = {
    ...
    ACCEPTED: { label: t('status.ACCEPTED'), color: STATUS_COLORS.ACCEPTED },
    ...
    CANCELED: { label: t('status.CANCELED'), color: STATUS_COLORS.CANCELED },
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="min-w-0">
            <h3 className="font-semibold truncate">
              {booking.businessService?.name || tc('prestation')}
            </h3>
...
export default function MesReservationsPage() {
  ...
  const cancelMutation = useMutation({
    ...
    onError: (err: Error) => {
      showError(err.message || t('cancelError'));
    },
  });
      ...
      <div className="container mx-auto px-4 py-16 pt-24 text-center">
        <div className="max-w-md mx-auto">
          ...
          <Link href="/auth/login">
            <Button size="lg" className="rounded-full px-8">{t('loginButton')}</Button>
          </Link>
        </div>
      </div>
    );
        ...
        {lastBookedBusiness && (
          <Link href={`/business/${lastBookedBusiness.slug}`}>
            <Button variant="outline" size="sm" className="rounded-full shrink-0">
              ...
              <span className="hidden sm:inline">{t('rebook')}</span>
              <span className="sm:hidden">{t('rebookShort')}</span>
            </Button>
          </Link>
        )}
      </div>
